from queuelinkedlist import LinkedQueue
...
class BinarySearchTree:
    ...
    def recurinsert(self, troot, e):
        '''Recursion insert in binary search tree'''
        if troot == None:
            node = self._Node(e)
            return node
        if e < troot._element:
...
print("recursion insert")
r = BinarySearchTree()
r._root = r.recurinsert(None, 70)
r.recurinsert(r._root, 30)
r.recurinsert(r._root, 90)
r.recurinsert(r._root, 40)
r.recurinsert(r._root, 50)
r.recurinsert(r._root, 110)
r.inorder(r._root)
print()
print(r.search(50))
print(r.search(60))